...
@flow(persist_result=True, name="km_indicator_share", flow_run_name=generate_flow_run_name)
def process_flow(
        job_id: str,
        execution_request: schemas.ExecuteRequest
) -> dict:
    return KommonitorProcess.execute_process_flow(KmIndicatorShare.run, job_id, execution_request)

class KmIndicatorShare(KommonitorProcess):
    process_flow = process_flow
    
    detailed_process_description = ProcessDescription(
        id="km_indicator_share",
        version="0.0.1",
        title="Anteil (Quotient zwischen Basis-Indikatoren und einem Referenzindikator)",
        ...
        additional_parameters=AdditionalProcessIOParameters(
            parameters=[
                Parameter(
                    ...
                    value=[{
                        "longTitle": "Anteil mehrerer Basisindikatoren von einem Referenzindikator",
